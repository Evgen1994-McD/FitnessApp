...
class DaysAdapter(var listener: Listener) : ListAdapter<DayModel, DaysAdapter.DayHolder>(MyComporator()) { // А вот сюда мы запишем компоратор который отвечает за сравнение элеентов. А так же сюда передаем листенер Интерфейс
    ...
    class DayHolder(view : View) : RecyclerView.ViewHolder(view){  // это старый знакомый ViewHolder
        ...
        fun setData(day :DayModel, listener: Listener) = with(binding) {  // прикольная фича. Чтобы не писать binding.tvName А писать сразу tvName. Круто! Напрямую
