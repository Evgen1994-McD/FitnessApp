...
var currentDay = 0 // переменная для того чтобы через DayFragment записывать текущий день в ш.преф
    ...
    fun getPref(key: String): Int {
       return pref?.getInt(key, 0) ?:0   // с помощью оператора элвиса мы выведем 0, в том случае, если преф не инициализирован. (?:) - оператор выдаёт то, что находится справа, если слева null
