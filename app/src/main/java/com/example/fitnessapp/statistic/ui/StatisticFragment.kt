        ...
        return ComposeView(requireContext()).apply {
            ...
            setContent {
                FitnessAppTheme {
                    ...
                    var eventList by remember { mutableStateOf(model.eventListData.value) }
                    var tempWeightList by remember { mutableStateOf(model.weightListData.value) }

                    // Следим за изменением исходных потоков данных
                    LaunchedEffect(key1 = Unit) {
                        snapshotFlow { Pair(model.eventListData.value, model.weightListData.value) }.collect { (events, weights) ->
                            eventList = events
                            tempWeightList = weights
                        }
                    }
                    ...
                    StatisticScreen(
                        date = "Сегодня",
                        eventList = eventList,
                        onDayClick = { model.getYearList() },
//                        tempWeightList = tempWeightList,
                         "Сегодня"
                    )
                }
            }
